...
import { FaPlus, FaTrash, FaUpload, FaStore, FaEdit } from 'react-icons/fa';
...
export default function SuppliesPage() {
  ...
  const handleEdit = (supply: Supply) => {
    ...
    setNewSupply({
      ...
      minimumOrderQuantity: supply.minimumOrderQuantity,
      supplier: {
        name: supply.supplier.name,
        contactInfo: supply.supplier.contactInfo || '',
        website: supply.supplier.website || ''
      }
    });
    setImageUrl(supply.image || '');
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update the supply
      const response = await fetch(`/api/admin/supplies/${editingSupply._id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {showEditModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold">Edit Supply</h2>
              <button
                onClick={() => {
                  setShowEditModal(false);
                  setEditingSupply(null);
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Enter unit (e.g., sheets, rolls)"
                    />
                    {formErrors.unit && (
                      <p className="mt-1 text-sm text-red-500">{formErrors.unit}</p>
                    )}
                ...
                <div className="mt-4">
                  ...
                  <textarea
                    ...
                    onChange={(e) => setNewSupply({ ...newSupply, description: e.target.value })}
                    rows={3}
                    className="w-full rounded-md shadow-sm border-gray-300 focus:border-purple-500 focus:ring-purple-500"
                    placeholder="Enter supply description"
                  />
                </div>
              </div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full rounded-md shadow-sm ${
                        formErrors['supplier.name'] ? 'border-red-500' : 'border-gray-300'
                      } focus:border-purple-500 focus:ring-purple-500`}
                      placeholder="Enter supplier name"
                    />
                    {formErrors['supplier.name'] && (
                      <p className="mt-1 text-sm text-red-500">{formErrors['supplier.name']}</p>
                    )}
                  </div>
