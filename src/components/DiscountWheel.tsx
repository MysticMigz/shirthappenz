    ...
    <div className="bg-gradient-to-r from-purple-600 to-blue-600 text-white py-2 px-4 overflow-hidden relative shadow-lg">
      <div className="w-full relative">
        ...
        <div className="overflow-hidden">
          <div className="flex items-center space-x-4 sm:space-x-6 animate-scroll">
            {/* Multiple repetitions for seamless looping */}
            {[...Array(4)].map((_, repeatIndex) => (
              <div key={repeatIndex} className="flex items-center space-x-1 sm:space-x-2 flex-shrink-0">
                <svg 
                ...
                <span className="font-medium text-xs sm:text-sm whitespace-nowrap">
                  <span className="font-bold text-yellow-300">{currentDiscount.code}</span> - <span className="hidden sm:inline">{currentDiscount.description}</span><span className="sm:hidden">Discount</span>
                </span>
      ...
      <button
        ...
        className="absolute right-2 sm:right-4 top-1/2 transform -translate-y-1/2 text-white hover:text-gray-200 transition-colors z-10 p-1"
